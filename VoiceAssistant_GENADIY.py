...
def record_and_recognize_audio(microphone, recognizer):
    with microphone:
        ...
        try:
            fraza=vlc.MediaPlayer()
            fraza.play()
            logging.info('Команда приветствия сработала')
...
def search_on_wikipedia(*args: tuple):
    ...
    try:
        if wiki_page.exists():
            ...
            play_voice_assistant_speech(f'На нахуй + {wiki_page.summary.split('.'[:2])}')
        else:
...
def get_weather(*args: tuple):
    ...
    temp=weather.temperature('ceilsius')["temp"]
...
def opera_open_first_window(*args: tuple):
    ...
    opera_path={'путь к опере на пк марка'}
    try:
